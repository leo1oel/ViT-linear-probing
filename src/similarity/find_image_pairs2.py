...
def load_features_to_gpu(
    file_path, device="cuda", load_batch_size=1000000, norm_batch_size=1000000
):
    """分批将特征加载到GPU并归一化"""
    with h5py.File(file_path, "r") as f:
        ...
        features = torch.empty(
            (total_size, feature_dim), dtype=torch.float32, device=device
        )
        paths = []

        for i in tqdm(range(0, total_size, load_batch_size), desc="Loading features"):
            end_idx = min(i + load_batch_size, total_size)
            batch_features = torch.from_numpy(f["last_hidden_cls"][i:end_idx]).to(
                device
            )
...
def find_contrasting_pairs_streamed(
    clip_features, dino_features, paths, idx1, idx2, batch_size=10000
):
    """流式处理相似度计算和对比对查找"""
    results = []
    n = len(idx1)
    ...
    for i in tqdm(range(0, n, batch_size), desc="Processing batches"):
        ...
        for j in range(0, n, batch_size):
            ...
            for mask, pair_type in [
                (mask1, "high_clip_low_dino"),
...
def find_contrasting_pairs_gpu(
    clip_file,
    dino_file,
    output_file,
    search_size=50000,
    target_pairs_clip=500,
    target_pairs_dino=1000,
    device="cuda",
    ...
    print(
        f"High CLIP, Low DINO pairs: {len(found_pairs_by_type['high_clip_low_dino'])}"
    )
    print(
        f"High DINO, Low CLIP pairs: {len(found_pairs_by_type['high_dino_low_clip'])}"
    )
    ...
    while (
        continue_search
        or len(found_pairs_by_type["high_clip_low_dino"]) < target_pairs_clip
        or len(found_pairs_by_type["high_dino_low_clip"]) < target_pairs_dino
    ):
        iteration += 1
        print(f"\nIteration {iteration}, processing batch of size {search_size}")
        print(
            f"Current counts - CLIP pairs: {len(found_pairs_by_type['high_clip_low_dino'])}, "
        ...
        with open(output_file, "a", newline="") as f:
            ...
            for pair in new_pairs:
                ...
                if not continue_search:
                    if (
                        pair["type"] == "high_clip_low_dino"
                        and len(found_pairs_by_type["high_clip_low_dino"])
                        >= target_pairs_clip
                    ):
                        continue
                    if (
                        pair["type"] == "high_dino_low_clip"
                        and len(found_pairs_by_type["high_dino_low_clip"])
                        >= target_pairs_dino
                    ):
                        continue
