...
def find_contrasting_pairs_batch(clip_features, dino_features, paths,
                               target_pairs=100, batch_size=100000,
                               clip_high_thresh=0.9, clip_low_thresh=0.2,
                               dino_high_thresh=0.95, dino_low_thresh=0.5,
                               k=2048,
                               gpu_id=0):
    """使用GPU在批次数据中找到对比的图片对"""
    ...
    for idx in tqdm(query_indices):
        ...
        if idx % 100 == 0:
            print(f"\nQuery {idx}:")
            print(f"CLIP similarities range: {clip_sim[0].min():.3f} to {clip_sim[0].max():.3f}")
            print(f"DINO similarities range: {dino_sim[0].min():.3f} to {dino_sim[0].max():.3f}")
        
        # 查找对比对
        for j in range(k):
            if clip_sim[0][j] > clip_high_thresh and dino_sim_for_clip[0][j] < dino_low_thresh:
            ...
            if dino_sim[0][j] > dino_high_thresh and clip_sim_for_dino[0][j] < clip_low_thresh:
        ...
        if idx % 100 == 0:
            print(f"Found {len(clip_high_dino_low)} CLIP-high pairs and {len(dino_high_clip_low)} DINO-high pairs")
        
        if len(clip_high_dino_low) >= target_pairs and len(dino_high_clip_low) >= target_pairs:
            break
...
def save_results(clip_high_pairs, dino_high_pairs, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['type', 'image1_path', 'image2_path', 
...
def main():
    ...
    print(f"Found {len(clip_high_pairs)} CLIP-high pairs and {len(dino_high_pairs)} DINO-high pairs")
