...
class ImageNetDataset(ImageDataset):
    ...
    def _setup(self):
        self.train_data = ImageFolderWithPaths(
            os.path.join(self.root, "train"), transform=self.transform
        )
        self.val_data = ImageFolderWithPaths(
            os.path.join(self.root, "val"), transform=self.transform
        )
...
class StanfordCarsDataset(ImageDataset):
    ...
    def _setup(self):
        ...
        self.train_data = StanfordCarsSubset(
            self.root, train_annos, "train", transform=self.transform
        )

        self.val_data = StanfordCarsSubset(
            self.root, test_annos, "test", transform=self.transform
        )
...
class CUBDataset(ImageDataset):
    ...
    def _setup(self):
        ...
        filename_to_label = {
            img[1]: int(label[1]) - 1 for img, label in zip(image_list, image_labels)
        }
        filename_to_split = {
            img[1]: int(split[1]) for img, split in zip(image_list, train_test_split)
        }

        class CUBSubset(datasets.VisionDataset):
            def __init__(
                self, root, filenames, filename_to_label, classes, transform=None
            ):
                super().__init__(root, transform=transform)
...
class ImageNetV2Dataset(ImageDataset):
    ...
    def _setup(self):
        self.train_data = None
        self.val_data = ImageFolderWithPaths(self.root, transform=self.transform)
...
def get_dataset(
    name: str, root: str, transform: Optional[transforms.Compose] = None
) -> ImageDataset:
    """获取指定的数据集"""
...
class FeatureDataset(Dataset):
        ...
        if normalize:
            if is_train:
                ...
                assert train_stats is not None, (
                    "Need training stats for validation/test set"
                )
    ...
    def print_statistics(self):
        ...
        console.print(
            f"Samples per class mean: {self.label_distribution.float().mean().item():.2f}"
        )
