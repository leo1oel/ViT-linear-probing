...
def cosine_lr(
    optimizer: optim.Optimizer, base_lrs: float, warmup_length: int, steps: int
):
...
def find_best_hyperparams(
    ...
    with Progress() as progress:
        ...
        search_task = progress.add_task(
            "[cyan]Searching hyperparameters...", total=total_combinations
        )
            ...
            for wd in weight_decays:
                ...
                for epoch in range(quick_epochs):
                    train_loss, train_acc = train_epoch(
                        model, train_loader, optimizer, None, device, epoch=epoch
                    )
                    console.print(
                        f"[dim]Quick Epoch {epoch + 1}/{quick_epochs}, Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.2f}%[/dim]"
                    )
...
def train_and_evaluate(
    ...
    train_dataset = FeatureDataset(
        train_features, train_labels, normalize=True, is_train=True
    )
    ...
    history = {"epochs": [], "train_loss": [], "train_acc": [], "val_acc": []}
